...
public class cr_redlamp_off1Service extends SEServiceImplementationThread {
	private static Logger logger = Logger.getLogger(cr_redlamp_off1Service.class);
	...
	public cr_redlamp_off1Service(ServiceWrapper service, BindingList b) {
		super(service, b);
	}
	...
	protected List<String> serviceImplementation() {
		if (Config.isRoleRegistered("GSE")){
			...
			Statement in = new StatementImpl(bnd.getInstance("http://www.dfki.de/isreal/cr_redlamp_off1.owl#self"), type, off);
			List<Statement> ins = new ArrayList<Statement>();
			ins.add(in);
			
			Statement out = new StatementImpl(bnd.getInstance("http://www.dfki.de/isreal/cr_redlamp_off1.owl#self"), type, on);
			...
			logger.error("No GlobalSE available in thread: " + cr_redlamp_off1Service.class.getName());
		}
		return new ArrayList<String>();
	}
