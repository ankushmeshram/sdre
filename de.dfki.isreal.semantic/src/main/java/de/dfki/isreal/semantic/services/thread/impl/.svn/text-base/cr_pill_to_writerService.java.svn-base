...
public class cr_pill_to_writerService extends SEServiceImplementationThread {
	private static Logger logger = Logger.getLogger(cr_pill_to_writerService.class);
	...
	public cr_pill_to_writerService(ServiceWrapper service, BindingList b) {
		super(service, b);
	}
	...
	protected List<String> serviceImplementation() {
		if (Config.isRoleRegistered("GSE")){
			...
			Statement st = new StatementImpl(bnd.getInstance("http://www.dfki.de/isreal/cr_pill_to_writer.owl#self"), hasColor, "?col");
			patterns.add(st);
			vars.add("?col");
			...
			if (col.equalsIgnoreCase("http://www.dfki.de/isreal/messemodul_abox.owl#BLUE")){
				graphics.sendAnimationCall("blue_pill_to_writer", null);
			}
			if (col.equalsIgnoreCase("http://www.dfki.de/isreal/messemodul_abox.owl#WHITE")){
				graphics.sendAnimationCall("white_pill_to_writer", null);
			}
			if (col.equalsIgnoreCase("http://www.dfki.de/isreal/messemodul_abox.owl#RED")){
				graphics.sendAnimationCall("red_pill_to_writer", null);
			}
			...
			logger.error("No GlobalSE available in thread: " + cr_pill_to_writerService.class.getName());
		}
		return new ArrayList<String>();
	}
