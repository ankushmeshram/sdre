...
public class cr_greenlamp_off2Service extends SEServiceImplementationThread {
	private static Logger logger = Logger.getLogger(cr_greenlamp_off2Service.class);
	...
	public cr_greenlamp_off2Service(ServiceWrapper service, BindingList b) {
		super(service, b);
	}
	...
	protected List<String> serviceImplementation() {
		if (Config.isRoleRegistered("GSE")){
			...
			Statement in = new StatementImpl(bnd.getInstance("http://www.dfki.de/isreal/cr_greenlamp_off2.owl#self"), type, off);
			List<Statement> ins = new ArrayList<Statement>();
			ins.add(in);
			
			Statement out = new StatementImpl(bnd.getInstance("http://www.dfki.de/isreal/cr_greenlamp_off2.owl#self"), type, on);
			...
			logger.error("No GlobalSE available in thread: " + cr_greenlamp_off2Service.class.getName());
		}
		return new ArrayList<String>();
	}
