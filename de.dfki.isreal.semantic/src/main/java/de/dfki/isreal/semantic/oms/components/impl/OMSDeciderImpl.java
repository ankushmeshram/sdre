...
public class OMSDeciderImpl implements OMSDecider {

	private Logger logger = Logger.getLogger(OMSDeciderImpl.class);
	...
	public OMSDeciderImpl(String loc) {
		...
		if (sem_r) {
			PSR = new SemanticReasonerPelletImpl();
		}
	}
	
	public OMSDeciderImpl(String loc, int jettyPort) {
		logger.info("init OMSDeciderImpl...");
		sem_r = OMSConfig.isSemReasoningEnabled();
		if (OMSConfig.getTripleStoreSystemName().equals("owlim")) {
